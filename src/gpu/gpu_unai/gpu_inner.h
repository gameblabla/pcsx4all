...
static void gpuSpriteSpanFn(u16 *pDst, u32 count, u8* pTxt, u32 u0)
{
  ...
  const u16 *CBA_;
  if (CF_TEXTMODE!=3) CBA_ = gpu_unai.CBA;
  ...
  do
  {
    ...
    if (CF_MASKCHECK) if (uDst&0x8000)
      {
        goto endsprite;
      }
...
static void gpuPolySpanFn(const gpu_unai_t &gpu_unai, u16 *pDst, u32 count)
{
  ...
  if (!CF_TEXTMODE)
  {
    ...
    else
    {
      ...
      do
      {
        ...
        if (CF_MASKCHECK)
        {
          if (uDst&0x8000) goto endpolynotextgou;
        }
  ...
  else
  {
    ...
    u32 l_u_msk = gpu_unai.u_msk;
    u32 l_v_msk = gpu_unai.v_msk;
    u32 l_u = gpu_unai.u & l_u_msk;
    u32 l_v = gpu_unai.v & l_v_msk;
    s32 l_u_inc = gpu_unai.u_inc;
    s32 l_v_inc = gpu_unai.v_inc;
