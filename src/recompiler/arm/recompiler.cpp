...
static void copia(void)
{
  memcpy((void *)&merde,(void *)&psxRegs,sizeof(psxRegisters));
}
static void restaura(void)
{
  memcpy((void *)&psxRegs,(void *)&merde,sizeof(psxRegisters));
}
#define recOpcode() { \
    if (rec_phase) { \
      write32(0xe92d001f); /* stmdb	sp!, {r0-r4} */ \
...
// CHUI: Si no hay que salvar contexto y no estamos en modo bloque tenemso que actualizar los registros
      if (encontrado)
        iUpdateRegs(0);
      else if (!((savectx)||(rec_total_opcodes>REC_MAX_OPCODES_LIMIT)))
        iUpdateRegs(0);
      CALLFunc((u32)psxBranchTestCalculate);
    }
...
//CHUI: Aqui sabemos que el salto a branchPC es correcto.
...
#endif
      {
        ...
        if ((rec_total_opcodes-rec_skips>=REC_MAX_SKIPS))
        {
          ...
          MOV32MtoR_regs(HOST_r0,(u32)&psxRegs.pc);
          j32Ptr[1]=armPtr;
          write32(B_FWD(0)); // Apuntamos el salto
...
// CHUI: Tenemos que controlar que nos pasamos de los 4096 max para el offset de los inmediatos
  ...
  {
    recPrev_jumps--;
    u32 offset=((((u32)(armPtr) - ((u32)(recPrevJump[recPrev_jumps].ptr) + (8))) >> 2) & 0xFFFFFF);
    *recPrevJump[recPrev_jumps].ptr = (B_FWD_(offset));
    ...
    else
    {
      ...
      iUpdateRegs(1);
      MOV32ItoR(HOST_r0,recPrevJump[recPrev_jumps].pc);
      MOV32RtoM_regs((u32)&psxRegs.pc,HOST_r0);
#ifdef REC_USE_RETURN_FUNCS
      JUMPFunc(func_TestBranchIfOK_ptr+4);
      UpdateImmediate(0);
