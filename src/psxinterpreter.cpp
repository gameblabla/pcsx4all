    ...
    case 0x00: // SPECIAL
      switch (_tFunct_)
      {
        ...
        case 0x03: // SRL/SRA
          ...
          else if (_tRt_ == reg) return 2;
          else if (_tRd_ == reg) return 3;
          break;
        ...
        case 0x07: // SLLV...
          ...
          else if (_tRt_ == reg || _tRs_ == reg) return 2;
          else if (_tRd_ == reg) return 3;
          break;
...
void psxADDI(void)
{
  if (!_Rt_) return;  // Rt = Rs + Im 	(Exception on Integer Overflow)
  _rRt_ = _u32(_rRs_) + _Imm_ ;
}
void psxADDIU(void)
{
  if (!_Rt_) return;  // Rt = Rs + Im
  _rRt_ = _u32(_rRs_) + _Imm_ ;
}
void psxANDI(void)
{
  if (!_Rt_) return;  // Rt = Rs And Im
  _rRt_ = _u32(_rRs_) & _ImmU_;
}
void psxORI(void)
{
  if (!_Rt_) return;  // Rt = Rs Or  Im
  _rRt_ = _u32(_rRs_) | _ImmU_;
}
void psxXORI(void)
{
  if (!_Rt_) return;  // Rt = Rs Xor Im
  _rRt_ = _u32(_rRs_) ^ _ImmU_;
}
void psxSLTI(void)
{
  if (!_Rt_) return;  // Rt = Rs < Im		(Signed)
  _rRt_ = _i32(_rRs_) < _Imm_ ;
}
void psxSLTIU(void)
{
  if (!_Rt_) return;  // Rt = Rs < Im		(Unsigned)
...
void psxADD(void)
{
  if (!_Rd_) return;  // Rd = Rs + Rt		(Exception on Integer Overflow)
  _rRd_ = _u32(_rRs_) + _u32(_rRt_);
}
void psxADDU(void)
{
  if (!_Rd_) return;  // Rd = Rs + Rt
  _rRd_ = _u32(_rRs_) + _u32(_rRt_);
}
void psxSUB(void)
{
...
void psxAND(void)
{
  if (!_Rd_) return;  // Rd = Rs And Rt
  _rRd_ = _u32(_rRs_) & _u32(_rRt_);
}
void psxOR(void)
{
  if (!_Rd_) return;  // Rd = Rs Or  Rt
  _rRd_ = _u32(_rRs_) | _u32(_rRt_);
}
void psxXOR(void)
{
  if (!_Rd_) return;  // Rd = Rs Xor Rt
  _rRd_ = _u32(_rRs_) ^ _u32(_rRt_);
}
void psxNOR(void)
{
  if (!_Rd_) return;  // Rd = Rs Nor Rt
  _rRd_ =~(_u32(_rRs_) | _u32(_rRt_));
}
void psxSLT(void)
{
  if (!_Rd_) return;  // Rd = Rs < Rt		(Signed)
  _rRd_ = _i32(_rRs_) < _i32(_rRt_);
}
void psxSLTU(void)
{
  if (!_Rd_) return;  // Rd = Rs < Rt		(Unsigned)
  _rRd_ = _u32(_rRs_) < _u32(_rRt_);
}
...
void psxSLL(void)
{
  if (!_Rd_) return;  // Rd = Rt << sa
  _u32(_rRd_) = _u32(_rRt_) << _Sa_;
}
void psxSRA(void)
{
  if (!_Rd_) return;  // Rd = Rt >> sa (arithmetic)
  _i32(_rRd_) = _i32(_rRt_) >> _Sa_;
}
void psxSRL(void)
{
  if (!_Rd_) return;  // Rd = Rt >> sa (logical)
  _u32(_rRd_) = _u32(_rRt_) >> _Sa_;
}
...
void psxSLLV(void)
{
  if (!_Rd_) return;  // Rd = Rt << rs
  _u32(_rRd_) = _u32(_rRt_) << _u32(_rRs_);
}
void psxSRAV(void)
{
  if (!_Rd_) return;  // Rd = Rt >> rs (arithmetic)
  _i32(_rRd_) = _i32(_rRt_) >> _u32(_rRs_);
}
void psxSRLV(void)
{
  if (!_Rd_) return;  // Rd = Rt >> rs (logical)
  _u32(_rRd_) = _u32(_rRt_) >> _u32(_rRs_);
}
...
void psxLUI(void)
{
  if (!_Rt_) return;  // Upper halfword of Rt = Im
...
void psxMFHI(void)
{
  if (!_Rd_) return;  // Rd = Hi
  _rRd_ = _rHi_;
}
void psxMFLO(void)
{
  if (!_Rd_) return;  // Rd = Lo
  _rRd_ = _rLo_;
}
...
void psxSB(void)
{
  psxMemWrite8 (_oB_, _rRt_ &   0xff);
}
void psxSH(void)
{
  psxMemWrite16(_oB_, _rRt_ & 0xffff);
}
void psxSW(void)
{
  psxMemWrite32(_oB_, _rRt_);
}
